...
describe("mobile-app service", () => {
  ...
  const backendActiveContext: ResidentAppContext = {
    ...
    context_label: resident.context_label,
  };
  ...
  it("hydrates the active context through the AccessOS me API", async () => {
    ...
    const snapshot: SessionSnapshot = {
      ...
      residentAuth: {
        ...
        cpf_digits: "07009718318",
        profile_type: "RESIDENT",
        active_context: backendActiveContext,
        contexts: [backendActiveContext],
      },
  ...
  it("checks bulletin module status before using the mural", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      "http://localhost:3000/bulletin/module-status?site_id=11",
      expect.objectContaining({
        method: "GET",
        headers: expect.objectContaining({
          Authorization: "Bearer access-token",
